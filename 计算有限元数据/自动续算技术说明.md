...
| `failure_registry.py` | 从 worker 日志恢复并原子维护 `failed_cases.json` |
...
HDF5 和 VTU 均未通过校验时，controller 还会检查 `failed_cases.json`。只有未被记录为失败的 case 才会进入 COMSOL worker。失败清单可由历史和当前 `worker_*.log` 中的“`Case_XXXX 计算失败`”记录恢复，因此升级脚本后无需手工整理旧失败编号。

状态优先级：

```text
有效 HDF5 > 有效 VTU > 已记录失败 > 待计算
```

有效输出优先于失败标记。某个失败 case 后续显式重算成功后，清单中的对应记录会自动清除。
...
for 尝试次数 in 1..max_retries+1:
    扫描 HDF5 和 VTU
    从 worker 日志同步 failed_cases.json
    pending = 同时缺少有效 HDF5/VTU 且未记录失败的 case
    if pending 为空:
        结束
...
每一轮都会重新读取文件系统和失败清单，不依赖内存中的“成功列表”。因此即使 controller 中断，下一次运行仍能从磁盘事实恢复。
...
默认重试单位不是固定原始 batch，而是重新扫描后仍可重试的 case。这样能够自动缩小集合：

- 同批成功项不会重算。
- 明确记录“计算失败”的 case 进入 `failed_cases.json`，不再自动重试。
- worker 在启动阶段崩溃、被关闭或超时，且没有形成明确 case 失败记录时，缺失项仍可重试。
- VTU 成功但转换失败项只重试转换。
- 损坏正式文件会被重新识别。

`--retry-failed` 会临时把失败清单中的 case 重新纳入队列，供模型修正后的人工验证使用。未指定该参数时，失败 case 不受 `--max-retries` 影响，始终跳过。

可计算项处理完但仍有失败 case 时，controller 写入 `complete_with_failures` 并返回退出码 1；仍有其他缺失项时写入 `incomplete`。

## 12. 状态文件一致性

`batch_state.json` 自身也采用临时文件加 `os.replace()` 更新。它是人和外部监控程序的进度接口，但不是断点续算的唯一依据。

恢复依据包括经过结构校验的 VTU/HDF5、`failed_cases.json` 和 worker 日志。即使状态文件或失败清单丢失，保留的 worker 日志仍能恢复明确失败记录。

## 13. 异常边界

| 异常位置 | 处理方式 |
|---|---|
| 单个 case 求解失败 | 记录完整堆栈，继续同批其他 case，写入失败清单并停止自动重试 |
...
- 有 VTU 无 HDF5 时只补转换。
- 已记录失败 case 默认跳过，只有显式添加 `--retry-failed` 才再次尝试。
...
```powershell
conda activate pinn
python -m py_compile .\计算有限元数据\failure_registry.py .\计算有限元数据\main.py .\计算有限元数据\run_remaining.py .\计算有限元数据\transfer2hdf5.py
```
...
2026-08-08 验证结果为自动化测试 9 项通过；真实计划扫描识别 1000 个总工况、870 个有效 HDF5、36 个明确失败并跳过、94 个待计算工况。
