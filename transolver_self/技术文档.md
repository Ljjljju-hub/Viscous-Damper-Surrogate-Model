...
std  = sqrt(max(var, epsilon^2))
x_hat = (x-mean)/std
x     = x_hat*std + mean
```

默认 `epsilon=1e-8`。统计量以 PyTorch buffer 保存：
...
训练阶段 `accumulate=True` 时更新；验证、推理和 rollout 使用 `accumulate=False`，只读取训练统计。所有统计随 `model_state_dict` 写入 checkpoint。
...
两个模型的 Normalizer 定义相同，但如果分别训练，统计结果严格一致仍要求：

1. 使用同一份冻结的数据文件；
2. 使用相同 train 划分；
3. 使用相同 batch size、随机种子和样本遍历顺序；
4. 在相同训练阶段开始和停止统计。

如果 COMSOL 文件在两次训练之间继续增加，两次统计就不会相同。最严格的对比方案是预先计算一次共享统计量，并加载到两个模型中。
...
验证指标直接复用 `meshGraphNet_self.train.evaluate()`：
...
`transolver_self/train.py` 复用 MeshGraphNet 的：
...
1. COMSOL 工况仍在生成，数据划分尚未冻结；
2. 尚未执行完整 100 epoch 对比实验；
3. Normalizer 当前在线累计，尚未独立保存统一统计文件；
...
| `meshGraphNet_self/utils/normalization.py` | 在线 Normalizer |
| `meshGraphNet_self/train.py` | 共享训练、指标和 checkpoint 工具 |
...
| `transolver_self/train.py` | Transolver 训练入口 |
| `transolver_self/rollout.py` | 自回归序列推理 |
| `transolver_self/test_pipeline.py` | Transolver 流程测试 |
| `meshGraphNet_self/training.py` | 两模型共享的可恢复训练层 |
| `meshGraphNet_self/experiment_utils.py` | manifest、RNG、CSV 原子写入工具 |
| `experiments/create_split_manifest.py` | 冻结数据快照与固定划分 |
| `experiments/run_scale_study.py` | 多模型、多规模、多种子自动调度 |
| `experiments/evaluate_experiment.py` | 单任务可恢复 rollout 评价 |
| `experiments/plot_scale_study.py` | 汇总均值、标准差与曲线 |
