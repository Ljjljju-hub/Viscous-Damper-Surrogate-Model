...
早停同时维护两个量：任何严格更低的 `valid_mse` 都会更新 `best.pt`；只有相对上一次显著最优值至少下降 0.2% 才会把早停等待计数清零。连续 10 轮未达到该阈值后停止。这样微小抖动不会无限延后早停，同时仍保留数值上真正最低的模型。
...
恢复内容包括模型、优化器、学习率调度器、epoch、global step、最佳验证损失和早停状态。旧 checkpoint 没有显著改善参考值时，会以其中的 `best_valid_loss` 作为兼容初值。
