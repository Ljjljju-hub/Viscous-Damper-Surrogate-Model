...
├── train.py                    # DataLoader、训练、验证、日志和存档
├── model/
...
├── test_mesh_motion.py         # 动网格公式测试
├── test_normalization.py       # 归一化器测试
├── test_training_pipeline.py   # Batch、前向、反向、推理测试
├── requirements.txt
└── README.md
```
...
```text
mean = sum/count
std  = sqrt(sum(x^2)/count - mean^2)
x_norm = (x-mean)/std
```

验证和推理阶段停止累计，只使用训练阶段统计值。所有统计量是模型 buffer，会随 `state_dict` 一起保存进 checkpoint。
...
默认自动选择 CUDA；CUDA 不可用时使用 CPU。也可以显式指定：

```powershell
python meshGraphNet_self/train.py --device cuda:0
python meshGraphNet_self/train.py --device cpu
```
...
日志默认写入 `meshGraphNet_self/runs`：

```powershell
conda activate pinn
tensorboard --logdir meshGraphNet_self/runs
```
...
1. 单步训练之外，`experiments/evaluate_experiment.py` 已实现 MeshGraphNet 完整时间序列 rollout。
2. 当前物理场只有 `p/T`；若 HDF5 增加流体速度，需要扩展 `field_names` 和模型输出维度。
3. `node_type` 当前全部为 `NORMAL`，尚未编码壁面或流固边界类型。
4. 普通训练仍支持按文件名的 80%/10%/10% 划分；正式实验可通过 `--split-manifest` 使用冻结且显式的 case 清单。
...
```powershell
conda activate pinn
python meshGraphNet_self/train.py --epochs 5 --batch-size 4 --num-workers 0 --device cuda:0
```
